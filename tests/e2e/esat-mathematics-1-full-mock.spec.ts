...
test("ESAT Mathematics 1 full mock opens natively and persists at the current viewport", async ({ page }) => {
  ...
  await expect(page.getByRole("list", { name: "Full Mocks" })).toContainText("27 questions · 40 minutes");

  await page.getByRole("link", { name: "Mathematics 1 full mock, 27 questions, 40 minutes. Start." }).click();
  ...
  await expect(page.getByText(/ORIGINAL FULL-LENGTH MOCK/u)).toBeVisible();
