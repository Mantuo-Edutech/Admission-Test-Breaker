...
test("ESAT Mathematics 2 full mock opens natively and persists at the current viewport", async ({ page }) => {
  ...
  await expect(page.getByRole("link", { name: /Mathematics 2.*MM1–MM8.*开始完整模考/u })).toBeVisible();

  await page.getByRole("link", { name: /Mathematics 2.*开始完整模考/u }).click();
  ...
  await expect(page.getByText(/MATHEMATICS 2.*ORIGINAL FULL-LENGTH MOCK.*满托原创完整模考/u)).toBeVisible();
