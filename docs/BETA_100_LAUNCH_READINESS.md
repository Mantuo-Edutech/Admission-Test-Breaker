...
- `pnpm supabase:auth-smtp:apply` 以供应商中立的方式写入并回读验证 Auth SMTP；host/port/发件身份存 GitHub Environment variables，user/password 只存 secrets，缺失时部署失败关闭。仍需开通供应商、认证 `auth.uktest.cc` 的 SPF/DKIM/DMARC，并完成真实注册确认与密码重置邮件；
...
- `pnpm production:preflight` 已把生产控制面变成独立失败关闭检查：只读取 GitHub Environment 中 secret 的名称和公开变量，不读取 secret 值；分别判断 GitHub setup 与当前 commit 是否为干净、已推送的 release candidate，并继续把真实邮件、Turnstile hostname、域名/TLS、平台恢复/告警/责任人列为人工证据。2026-07-24 的只读运行确认两个 Environment、两套 Supabase secret、正式 origin 与 production reviewer 已就绪；两套 Turnstile secret、SMTP user/password 与八个 SMTP 非秘密变量仍缺失。
- `production:bootstrap-plan`/`production:bootstrap-apply` 已把上述 GitHub 控制面配置变为默认 dry-run、显式确认才写入的重复流程。配置文件只有公开 origin、SMTP 非秘密身份、审批人和 secret 来源环境变量名；执行前十个 secret 少一个都会零写入，真实值只经 stdin 进入 GitHub CLI，失败即停，完成后由独立只读 gate 复核。它不会代替创建 SMTP/Turnstile 账号和 widget、DNS 认证、备份、告警和责任人。
