...
公开练习内容的目标交付边界见 ADR `2026-07-23-server-delivered-practice-content.md`：生产系统继续以 Supabase/PostgreSQL 为权威存储，客户端只取得当前练习需要的无答案 payload；Axum、HTMX 与独立仓库均保留为有实际负载或团队边界后再采用的实现选择，而不是先行增加的基础复杂度。
...
ESAT 的公开 `verify:esat-assets` 由 `scripts/check-esat-source-inventory.ts` 交叉核对 39 个来源 URL、research asset ID、私有路径、byte count、SHA-256、权利状态和 public summary，但绝不打开 raw 文件。`verify:esat-raw-assets` 则只在内部教研机器逐文件复算原件；生产 CI 不运行它，也不能以 tracked metadata 的通过声称本机原件仍存在。
...
练习与增值解析的关系也属于产品目录，而不是结果页文案。`review-notes` 可以用 `relatedPracticeIds` 指向一项或多项真实练习；结果页只有在公开 invite 产品、全部题目解析资源 ID 与产品 ID 完全一致时才展示转化入口，否则失败关闭。免费作答、答案和基础结果始终独立可用。锁定状态同时提供冰冰二维码和已有邀请码入口；邀请码、注册、登录或邮件确认只在 session 中保留经过双重校验的站内返回路径，兑换后立即清除。`/access` 在公开预检后读取认证状态：已登录学生直接核销，未登录学生才进入注册或登录。邮箱确认若换浏览器导致 pending code 不存在，只报告邮箱成功并要求重新输入原 code，不得把认证成功冒充 entitlement 成功；明文 code 仍不进入 URL。返回路径从不替代服务端 entitlement 检查。完整决策见 `docs/architecture/decisions/2026-07-19-catalog-linked-review-conversion.md`。
...
`deploy/bootstrap-requirements.json` 是生产控制面的最小机器契约。`pnpm production:preflight` 只通过 read-only git、GitHub API 和 Docker 查询检查仓库、四个部署工作流、staging/production Environment、所需 secret 的名称、公开 HTTPS origin、SMTP 非秘密身份变量、production required reviewer，以及 `main` 是否强制 PR、最新 application/database 双检查、管理员不可绕过、禁止强推和删除；发布候选还必须位于 main、干净、已推送并拥有同一 SHA 的成功 `Verify` 运行。它不读取 secret 值，也没有创建/修改 Environment 或分支保护的代码路径。GitHub setup、release candidate 与最终 Beta readiness 是三个不同状态：前两者通过仍不能替代 Supabase 项目/MFA、真实邮件交付、Turnstile hostname、域名/TLS、备份/告警/责任人和 `beta:gate`。真实状态必须以当前 `pnpm production:preflight` 输出为准，不在架构文档中固化易过期计数。

写入路径与预检保持物理分离。`deploy/bootstrap-input.local.json` 只记录公开 origin、SMTP 非秘密身份、GitHub reviewer 和 secret 的本机环境变量来源名，并被 Git 忽略；`production:bootstrap-plan` 默认不读取真实值、不调用写 API。只有 `production:bootstrap-apply` 加精确确认文字才进入执行器，且执行器在任何外部调用前一次性验证全部十个 secret 来源。secret 通过子进程 stdin 交给 `gh secret set`，不进入 argv、日志或报告；Environment、公开 variable 与 reviewer 写入失败会立即停止，最后再调用独立只读 bootstrap gate 复核。该路径解决可重复配置，不创造 SMTP 账号、发件域名 DNS、Turnstile widget 或任何人工生产证据；决策见 `docs/architecture/decisions/2026-07-19-explicit-production-bootstrap-apply.md` 与 `docs/architecture/decisions/2026-07-24-fail-closed-custom-auth-smtp.md`。
