...
该命令缺少生产 secret 时仍可安全运行，并逐项显示“尚无证据、过期、release 不匹配或控制源已变化”。生产部署后，证据工作树必须准确 checkout 到已部署的 40 位 commit SHA；除 `verification/production/evidence/` 下的隐私安全报告外不得存在任何修改。记录器会读取 Git HEAD 和 porcelain status 独立复核，拒绝用未来分支的验证代码替旧 release 背书。确认后再逐项执行自动控制：
...
`remote-learner-data-contract` 不是单卷抽查：它会逐一读取发布清单中的全部试卷，验证每套题的准确 revision、content digest、考试、题数、时长和无答案公开边界，并强制 18 套 TMUA 历年卷全部存在；随后才使用一个临时双账号样本验证 Session 持久化、基础评分、RLS、邀请码和 Notes 隔离，最后删除临时账号。
