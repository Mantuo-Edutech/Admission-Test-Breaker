...
| 基础安全访问日志 | 防攻击、诊断故障和调查安全事件 | 与产品漏斗分离；宿主机可能记录 IP、请求路径、时间、状态、来源页和浏览器信息；当前 ECS 日志每日轮转、保留 14 份，不用于学习画像 |
...
生产审核的固定决策项、处理链事实表、技术证据映射与去标识化签署模板见
`docs/legal/PRODUCTION_PRIVACY_REVIEW_PACKET.md`。该 packet 用于让真实审核人逐项作出决定，
不会把工程团队准备材料替代为法律批准。
